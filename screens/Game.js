...
const makeAIMove = (currentBoard) => {
  ...
  const declareWinner = async (player) => {
    if (winner) return; // ตรวจสอบว่ามีผู้ชนะแล้วหรือยัง
    
    setWinner(player); // ตั้งค่าผู้ชนะ
    ...
    //     result,
    //     gridSize,
    //     moves: [...moveHistory], // ใช้ spread operator เพื่อบันทึก copy ของ moveHistory
    //   };
    ...
    console.log('สรุป move history:', moveHistory);
    setTimeout(() => {
      resetBoard(); // รีเซ็ตกระดานหลังจาก 2 วินาที
    }, 2000);
  };
